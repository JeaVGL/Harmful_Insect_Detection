    ...
    try:
        ...
        for detail in input_details + output_details:
            if detail['dtype'] != np.int8:
                all_int8 = False
                print(f"  Tensor {detail['name']} is {detail['dtype']}, not INT8")
        
        if all_int8:
            print("  All tensors are INT8 - compatible with TFLite Micro!")
        else:
            print("  Some tensors are not INT8 - may cause compatibility issues")
        ...
        if mse < 0.01 and mae < 0.1:  # Thresholds can be adjusted
            ...
            print("  Quantization quality may be degraded")
